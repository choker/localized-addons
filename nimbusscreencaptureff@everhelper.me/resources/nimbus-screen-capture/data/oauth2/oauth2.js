 ...
 */
OAuth2.adapters = {};
OAuth2.adapterReverse = localStorage.oauth2_adapterReverse && JSON.parse(localStorage.oauth2_adapterReverse) || {};
...
OAuth2.prototype.updateLocalStorage = function() {
    ...
    var data = {};
    var variables = [
        'accessToken', 'accessTokenDate', 'apiScope', 'clientId', 'clientSecret', 'expiresIn', 'refreshToken'
    ];
...
OAuth2.prototype.openAuthorizationCodePopup = function(callback) {
    ...
    // it later.
    self.port.emit("send", {type: 'setworker'});
    window['oauth-callback'] = callback;

    // Create a new tab with the OAuth 2.0 prompt
    window.open(this.adapter.authorizationCodeURL(this.getConfig()));
//    chrome.tabs.create({url: this.adapter.authorizationCodeURL(this.getConfig())}, function(tab) {
...
OAuth2.prototype.getAccessAndRefreshTokens = function(authorizationCode, callback) {
    ...
    that.sendMessage({type:'access',code:items['code']}, function(message) {
        console.log(message);
        callback(that.adapter.parseAccessToken(message));
    });
...
OAuth2.prototype.refreshAccessToken = function(refreshToken, callback) {
    ...
    this.sendMessage({type:'refresh',token:refreshToken}, function(message) {
        console.log(message);
        var obj = message;
...
//        }
        ...
        that.sendMessage({'type':'finished','error':error});
...
OAuth2.loadAdapter = function(adapterName, callback) {
    ...
    script.src = '/nimbus-screen-capture/data/oauth2/adapters/' + adapterName + '.js';
...
OAuth2.prototype.getUploadFolder = function() {
    return !!localStorage['oauth2_upload_folder'] ? JSON.parse(localStorage['oauth2_upload_folder']) : {id: 'root', title: 'My Drive'};
};

OAuth2.prototype.setUploadFolder = function(folder) {
    if (folder !== undefined)
        localStorage['oauth2_upload_folder'] = JSON.stringify(folder);
};

OAuth2.prototype.sendMessage = function(data, cb) {

    if(typeof cb === 'function'){
        self.port.once("request", function(message){
            cb(message);
        });
    }
